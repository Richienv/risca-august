...
function ScreenshotSection({ screenshotRef }: { screenshotRef: React.RefObject<HTMLDivElement> }) {
  return (
    <section className="relative z-10 container mx-auto px-4 sm:px-6 md:px-8 lg:px-10 mt-10 sm:mt-12 md:mt-14">
      <div ref={screenshotRef} className="bg-gray-900 rounded-xl overflow-hidden shadow-2xl border border-gray-700/50 w-full sm:w-[90%] md:w-[80%] lg:w-[70%] xl:w-[65%] mx-auto">
        <div>
          <Image
...
function HeroContent() {
  return (
    <div className="text-left text-white pt-16 sm:pt-20 md:pt-24 lg:pt-32 xl:pt-40 px-4 sm:px-6 md:px-8 w-full max-w-sm sm:max-w-xl md:max-w-2xl lg:max-w-4xl xl:max-w-5xl">
      <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold mb-4 sm:mb-5 md:mb-6 leading-tight tracking-wide">
        Cuman 150 Hari <br className="sm:hidden" />Punya Penghasilan<br className="sm:hidden" /> 2 Digit
      </h1>
      <p className="text-base sm:text-lg md:text-xl lg:text-xl mb-6 sm:mb-7 md:mb-8 opacity-80 max-w-full sm:max-w-lg md:max-w-xl lg:max-w-2xl">
        Modal pelatihan dan mentorship bisa ubah kamu dari diam dirumah nonton drakor jadi penghasilan 2-3 digit passive income setiap bulannya!
      </p>
      <div className="flex pointer-events-auto flex-col sm:flex-row items-start sm:items-center space-y-3 sm:space-y-0 sm:space-x-4 md:space-x-6">
        <button className="bg-[#8200DB29] hover:bg-black/50 text-white font-semibold py-2.5 sm:py-3 md:py-3.5 px-6 sm:px-7 md:px-8 lg:px-10 rounded-full transition duration-300 w-full sm:w-auto text-sm sm:text-base md:text-lg border border-[#322D36]" style={{ backdropFilter: 'blur(8px)' }}>
          Saya Mau Perubahan
        </button>
        <button className="pointer-events-auto bg-[#0009] border border-gray-600 hover:border-gray-400 text-gray-200 hover:text-white font-medium py-2.5 sm:py-3 md:py-3.5 px-6 sm:px-7 md:px-8 lg:px-10 rounded-full transition duration-300 flex items-center justify-center w-full sm:w-auto text-sm sm:text-base md:text-lg">
    ...
    <nav className="fixed top-0 left-0 right-0 z-20" style={{ backgroundColor: 'rgba(13, 13, 24, 0.3)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', borderRadius: '0 0 15px 15px' }}>
      <div className="container mx-auto px-4 sm:px-6 md:px-8 lg:px-10 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4 md:space-x-6">
          <a href="#" className="hidden md:block text-gray-300 hover:text-white text-sm transition-colors">Contact Sales</a>
          <a href="#" className="hidden sm:block text-gray-300 hover:text-white text-sm transition-colors">Sign In</a>
          <a href="#" className="bg-[#8200DB29] hover:bg-black/50 text-white font-semibold py-2 px-5 rounded-full text-sm md:text-base border border-[#322D36] transition-all" style={{ backdropFilter: 'blur(8px)' }}>Start Free Trial</a>
  ...
  return (
    <div className="relative">
      ...
      <div className="bg-black relative z-10" style={{ marginTop: '-10vh' }}>
        ...
        <div className="container mx-auto px-4 sm:px-6 md:px-8 lg:px-10 py-16 text-white">
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-center mb-8">Other Content Below</h2>
             <p className="text-center max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl mx-auto opacity-80 text-base sm:text-lg">This is where additional sections of your landing page would go.</p>
        </div>
      </div>
    </div>
  );
